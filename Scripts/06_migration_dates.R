...
#' Load hex polygons and birds count (could load bird hexes, but takes much longer to run)
...
#' band of the predicted daily occurrence as a threshold to define BREEDING
...
g <- list()
for(s in c("Eastern Bluebird", "Mountain Bluebird", "Western Bluebird")) {
...
for(i in 1:nrow(birds)) {
  ...
  # spring threshold latitude for migration
...
#+ fig.asp = 2, fig.width = 10, out.width = "75%"
wrap_plots(g) + plot_layout(guides = "collect", nrow = 1)
